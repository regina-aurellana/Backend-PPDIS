...
class TakeOffTableController extends Controller
{
    ...
    // private function rsbDupa(){

    //     info('ur here');

    // }

    public function dupaPerProjectList($b3_project_id)
    {
        // $dupa_per_project = DupaPerProject::where('b3_project_id', $b3_project_id)->get();

        // return $dupa_per_project;
    ...
    public function selectDupaTakeOff(Request $request)
    {
        try {
            ...
            foreach ($table_set as $key => $table) {
                ...
                $dupaPerProject = TableDupaComponentFormula::join('table_dupa_components', 'table_dupa_component_formulas.table_dupa_component_id', '=', 'table_dupa_components.id')
                    ->join('dupa_per_projects', 'table_dupa_components.dupa_id', '=', 'dupa_per_projects.dupa_id')
                    ->where('table_dupa_component_formulas.id', $table_component_id)
                    ->first(['dupa_per_projects.*']);

                //  info($dupaPerProject->item_number);

                // if($dupaPerProject->item_number === '902 (1) a'){
    ...
    public function takeOffTablebyTakeoffID($take_off_table)
    {
    ...
    public function contingency(Request $request, TakeOffTable $take_off_table)
    {
        ...
        try {
            //UPDATE CONTINGENCY IN TAKE OFF TABLE
            $take_off_table_contingency = $take_off_table->update([
    ...
    public function transferTakeOffDupaQuantityToPOW(TakeOffTable $take_off_table, TakeOffService $service)
    {
        $take_off_table_var = TakeOffTable::where('id', $take_off_table->id)->first();

        if ($take_off_table_var) {
            $service->updatePOWDupaQuantityAndCost($take_off_table->take_off_id);
        }
    }
